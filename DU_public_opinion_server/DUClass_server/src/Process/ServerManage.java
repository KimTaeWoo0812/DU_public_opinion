...
//			System.out.println("현재 " + ProcessThread.clients.size() + "명이 접속 중입니다.");

			break;
			
		case "notice":
			temp = in.nextLine();
			ProcessThread.notice = temp;
			System.out.println("공지 변경: " + temp);
			ProcessThread.hasNotice = true;
			break;
		case "showNotice":
			System.out.println("공지 내용: " + ProcessThread.notice);
			break;
		case "reNotice":
			...
			if(temp.compareTo("Y") == 0||temp.compareTo("y") == 0){
				ProcessThread.notice = "";
				ProcessThread.hasNotice = false;
			}
			break;
		case "getNotice":
			System.out.println("DB에서 공지 가져오기 .. hasNotis = true ");
			ProcessThread.notice=db.getNotice();
			ProcessThread.hasNotice = true;
